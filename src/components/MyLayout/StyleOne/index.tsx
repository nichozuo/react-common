...
export default ({
  ...
  // 初始化主菜单数据
  const [mainMenus] = useState<MenuItemType[]>(() => getMainMenus(auth.permissions?.children));
  ...
  return (
    <Layout className="my-layout">
      <Layout.Sider className="my-layout-sider" width={100}>
      ...
      <Layout>
        <Layout.Header className="my-layout-header">
          <div className="my-layout-title">{setting?.title || '后台管理系统'}</div>
          <Dropdown
            overlay={
              <Menu
                onClick={(e: any) => {
          ...
          >
            <div className="my-avatar-wrap">
              <Avatar className="my-avatar">{auth.user?.username}</Avatar>
        ...
        <Layout>
          <Layout.Sider width={150} className="my-layout-subsider">
            <SubMenu
              menus={subMenus}
              defaultSelectedKeys={defaultSelectedKeys}
              fieldNames={fieldNames}
